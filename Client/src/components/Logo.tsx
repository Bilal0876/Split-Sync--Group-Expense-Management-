...
const Logo = ({ variant = 'horizontal' }: LogoProps) => {
     ...
     return (
          <div className={`flex ${isVertical ? 'flex-col' : 'items-center'} items-center gap-2.5`}>
               <span className={`${isVertical ? 'w-8 h-8 sm:w-10 sm:h-10' : 'w-9 h-9 sm:w-10 sm:h-10'} rounded-xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-violet-500/30 transition-all`}>
